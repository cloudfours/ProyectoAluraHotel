...
public class EditarDao {
	...
	public static int modificar(String nombre, String apellido, String fecha, Integer idPais, Integer telefono,
			Integer idReserva, int id) {
		final Connection con = new ConexionPool().crearConexion();
		try {
			try (con) {
				final PreparedStatement statement = con.prepareStatement(
						"UPDATE huespedes set nombre = ?,apellido = ?,fecha_de_nacimiento = ? ,id_pais= ? ,telefono= ? ,id_reserver= ? "
								+ "where id= ? ");
				try (statement) {
	...
	public int modificarRe(String fechaEntreada, String Fechasalida, Float valor, Integer idPago, int id) {
		...
		try {
			try (con) {
				final PreparedStatement statement = con.prepareStatement(
						"UPDATE reservas set fecha_entrada = ?,fecha_salida = ?,valor = ? ,id_pago= ? " + "where id= ? ");
				try (statement) {
